...
es_callback = tf.keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=20, verbose=0, mode='auto', baseline=None, restore_best_weights=True)

checkpoint_path = '/projectnb/nickar/sidd/code/uhoh/cp.ckpt'
...
def knn_(units):
    ...
    for i in range(len(train_features)):
        fea = train_features[i]
        train_features1.append(np.squeeze(fea).tolist())
        train_class1.append(train_class[i])
    test_features1 = []
    test_class1 = []
    for i in range(len(test_features)):
        fea = test_features[i]
        test_features1.append(np.squeeze(fea).tolist())
        test_class1.append(test_class[i])
    
    scores = []
    index = []
    for i in range(123):
        knn = KNN(n_neighbors=i+1)
        knn.fit(train_features1, train_class1)
        scores.append(knn.score(test_features1,test_class1))
        index.append(i+1)
